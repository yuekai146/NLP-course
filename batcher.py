  ...
  def init_encoder_seq(self, example_list, hps):
    """Initializes the following:
        ...
        self.enc_padding_mask:
          numpy array of shape (batch_size, <=max_enc_steps), containing 1s and 0s. 1s correspond to real tokens in enc_batch and target_batch; 
  ...
  def init_decoder_seq(self, example_list, hps):
    """Initializes the following:
        ...
        self.dec_padding_mask:
          numpy array of shape (batch_size, max_dec_steps), containing 1s and 0s. 1s correspond to real tokens in dec_batch and target_batch; 
          0s correspond to padding.
        """
...
class Batcher(object):
  ...
  def next_batch(self):
    ...
    if self._batch_queue.qsize() == 0:
      message = 'Bucket input queue is empty when calling next_batch. Bucket queue size: {0}, Input queue size: {1}'.format(self._batch_queue.qsize(), self._example_queue.qsize())
  ...
  def fill_example_queue(self):
    ...
    while True:
      ...
      abstract_sentences = [sent.strip() for sent in data.abstract2sents(str(abstract))] # Use the <s> and </s> tags in abstract to get a list of sentences.
    ...
    while True:
      ...
      for idx,t in enumerate(self._example_q_threads):
        if not t.is_alive(): # if the thread is dead
          raise('Found example queue thread dead. Restarting.')
      ...
      for idx,t in enumerate(self._batch_q_threads):
        if not t.is_alive(): # if the thread is dead
          raise('Found batch queue thread dead. Restarting.')
  ...
  def text_generator(self, example_generator):
    ...
    while True:
      ...
      try:
        ...
        raise('Failed to get article or abstract from example')
        continue
      if len(article_text)==0: # See https://github.com/abisee/pointer-generator/issues/1
        warnings.warn('Found an example with empty article text. Skipping it.')
      else:
